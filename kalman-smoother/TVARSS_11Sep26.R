...
TVARSS <- function(X, p = 1, ME = NULL, U = NULL, initial.points = "stationary", b0.start = NA, b.start = array(NA, dim = p), se.start = NA, su.start = 0.01, sb0.start = .05, sb.start = array(0.05, dim = p), c.start = if(is.null(U)) NULL else array(0, dim=dim(U)[2]), d.start = if(is.null(U)) NULL else array(0, dim=dim(U)[2]), b0.fixed = NA, b.fixed = array(NA, dim = p), se.fixed = NA, su.fixed = NA, sb0.fixed = NA, sb.fixed = array(NA, dim = p), c.fixed = if(is.null(U)) NULL else array(NA, dim=dim(U)[2]), d.fixed = if(is.null(U)) NULL else array(0, dim=dim(U)[2]), Tsamplefract = .9, annealing = F, show.count = 10^10, show.fig = T, method = "Nelder-Mead", maxit.BFGS = 10^4, maxit.SANN = 10^2, optim.BFGS.control = NULL, optim.SANN.control = NULL) {
	...
	if(opt$convergence != 0) print("convergence failed")
...
TVARSS_KalmanSmoother <- function(mod, show.fig = F){
	...
	xs.se <- matrix(sqrt(pmax(Vs, 0)), ncol = 1)
...
# simulate.TVARSS <- function(mod){
	# ####################################################
	# # Begin TVARSS_sim
	# TVARSS_sim <- function(X.fitted, X.mean.fitted, b0.fitted, b.fitted, ME, U, p, par.full) {
		...
		# # information saved
		# Xlist <- matrix(NA, nrow=Tmax, ncol=1)	
		# Xlist[1:p] <- X.fitted[1:p]

		# meanX <- matrix(NA, nrow=Tmax, ncol=1)
		# meanX[1:p] <- X.mean.fitted[1:p]
			
		# Selist <- matrix(NA, nrow=Tmax, 1)
		# Sulist <- matrix(NA, nrow=Tmax, 1)
		...
		# for(t in (t.start+1):Tmax) {
			...
			# if(!is.null(U)){
				# x[1] <- x[1] + U[t,] %*% cc
			# }
			# rSe <- rnorm(sd = abs(se), n = 1)
			# x[1] <- x[1] + rSe
			# Selist[t] <- rSe
			
			# rSu <- rnorm(sd = abs(su), n = 1)
			# if(is.null(U)){
				# Xlist[t] <- x[1] + rSu
				# meanX[t] <- B0
			# }else{	
				# Xlist[t] <- x[1] + as.numeric(U[t,,drop=F] %*% dd) + rSu
				# meanX[t] <- B0 + U[t,,drop=F] %*% cc + U[t,,drop=F] %*% dd
			# }
			# Sulist[t] <- rSu
		# }
		# df <- data.frame(time = 1:t, X = Xlist, X.mean = meanX, se = Selist, su = Sulist)
		# return(df)
	# }	
	# # End TVARSS_sim
	# ####################################################
	# # simple interpolation of b0 and b
	# Tmax <- dim(mod$X.fitted)[1]
	
	# b0.fitted <- mod$b0.fitted
	# b0.fitted[is.na(b0.fitted)] <- approx(y = b0.fitted, x = (1:Tmax), xout = (1:Tmax)[is.na(b0.fitted)])$y

	# b.fitted <- mod$b.fitted
	# for(j in 1:ncol(b.fitted)){
		# b.fitted[is.na(b.fitted[,j]), j] <- approx(y = b.fitted[,j], x = (1:Tmax), xout = (1:Tmax)[is.na(b.fitted[,j])])$y
	# }
	
	# df <- TVARSS_sim(X = mod$X.fitted, X.mean.fitted = mod$X.mean, b0.fitted = b0.fitted, b.fitted = b.fitted, ME = mod$ME, U = mod$U, p = mod$p, par.full = mod$par.full)
	
	# # remove points that were not sampled in mod$X.fitted
	# df$X[is.na(mod$X.fitted)] <- NA
...
P_indep_vars_TVARSS <- function(mod){
	if(!is.null(colnames(mod$U))){
		out <- data.frame(var = colnames(mod$U))
		out.logLik <- data.frame(var = colnames(mod$U))
	}else{
		out <- data.frame(var = 1:ncol(mod$U))
		out.logLik <- data.frame(var = 1:ncol(mod$U))
	}		
	if(ncol(mod$U) == 1){
		mod0 <- TVARSS(X = mod$X, ME = mod$ME, p = mod$p, Tsamplefract = .9, show.fig = T, annealing = F,
			initial.points = mod$initial.points,
		...
		out$dev[1] <- dev
		out$P[1] <- P	
		out.logLik$mod[1] <- mod$logLik	
		out.logLik$mod0[1] <- mod0$logLik	
		...
		for(i.col in 1:ncol(mod$U)){
			...
			out$dev[i.col] <- dev
			out$P[i.col] <- P		
			out.logLik$mod[i.col] <- mod$logLik	
			out.logLik$mod0[i.col] <- mod0$logLik	
		}
	}
	df <- out[,-1]
	rownames(df) <- out[,1]

	return(list(coef=df, logLik = out.logLik))
}

# simple code for producing a list of P-values from WALD
# ONLY WORKS FOR c, NOT d
PWald_indep_vars_TVARSS <- function(mod){
	
	####################################################
	# Begin TVARSS.ml
	TVARSS_ml <- function(par, X, ME, U, p, par.fixed, initial.points, show.count = 10^10, show.fig = T, LL.only = T, min.x) {
	...
	par.full <- mod$par.fixed
	par.full[is.na(mod$par.fixed)] <- mod$opt.par
	...
	par.for.se <- cc
	par.fixed.for.se <- par.full
	pick <- (p+4+p+1):(p+4+p+nu)
	par.fixed.for.se[pick] <- NA

	H <- numDeriv::hessian(func = TVARSS_ml, x = par.for.se, par.fixed = par.fixed.for.se, X = mod$X, ME = mod$ME, U = mod$U, p = mod$p, initial.points = mod$initial.points, min.x = mod$min.x)
	...
	nu <- ncol(mod$U)
	df <- data.frame(est = par.full[pick], se = diag(inv.information.matrix)^0.5)
	df$P <- 2*pnorm(abs(df$est), sd = df$se, lower.tail = F)
	rownames(df) <- colnames(U)

	return(list(coef = df, inv.information.matrix = inv.information.matrix))
}
